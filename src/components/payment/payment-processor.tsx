...
import { PaymentLink, LinkType, AmountType, UsageType, PaymentRequest } from '@/types/relynk';
import { SUPPORTED_TOKENS } from '@/lib/contracts';
import { toast } from 'sonner';
import { Loader2, CreditCard, Heart, ShoppingBag, FileText, ExternalLink, CheckCircle } from 'lucide-react';
...
export function PaymentProcessor({ paymentLink, onSuccess, onError }: PaymentProcessorProps) {
  ...
  const { data: balance } = useBalance({
    address,
    token: paymentLink.token === '0x0000000000000000000000000000000000000000' ? undefined : paymentLink.token,
  });
  ...
  const getTokenDecimals = (tokenSymbol: string): number => {
    const token = Object.values(SUPPORTED_TOKENS).find(t => t.symbol === tokenSymbol);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const paymentRequest: PaymentRequest = {
        ...
        message: message,
        customAmount: paymentLink.amountType !== AmountType.FIXED ? amountToCharge : undefined,
      };
  ...
  // Check if link is already used (for single-use links)
  const isUsed = Boolean(isLinkUsed) && paymentLink.usageType === UsageType.ONE_TIME;
        ...
        <CardContent className="pt-6">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              {(paymentLink.metadata.linkType === 0 && 'successMessage' in paymentLink.metadata) 
                ? paymentLink.metadata.successMessage || 'Your payment has been processed successfully.'
                : 'Your payment has been processed successfully.'}
            </p>
            {(paymentLink.metadata.linkType === 0 && 'redirectUrl' in paymentLink.metadata && paymentLink.metadata.redirectUrl) && (
              <Button asChild className="w-full">
                <a href={paymentLink.metadata.redirectUrl} target="_blank" rel="noopener noreferrer">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="font-medium">
              {`${paymentLink.creator.slice(0, 6)}...${paymentLink.creator.slice(-4)}`}
            </p>
            ...
            <div className="space-y-2">
              ...
              <div className="flex">
                <Input
                  ...
                  type="number"
                  step="0.000001"
                  placeholder={paymentLink.amountType === AmountType.DYNAMIC ? '0.00' : paymentLink.amount}
                  value={customAmount}
                  onChange={(e) => setCustomAmount(e.target.value)}
                  className="rounded-r-none"
                />
                <div className="flex items-center px-3 border border-l-0 rounded-r-md bg-muted">
                  <span className="text-sm font-medium">{paymentLink.tokenSymbol}</span>
                </div>
              </div>
            </div>
          )}
        ...
        {balance && (
          <div className="text-sm text-muted-foreground text-center">
            Your balance: {formatEther(balance.value)} {paymentLink.tokenSymbol}
          </div>
        )}
        ...
        >
          {isProcessing ? (
            ...
            <>
              {getIcon()}
              <span className="ml-2">
                {paymentLink.linkType === LinkType.DONATION ? 'Donate' : 'Pay'} 
                {paymentLink.amountType === AmountType.FIXED && ` ${paymentLink.amount} ${paymentLink.tokenSymbol}`}
              </span>
            </>
          )}
        </Button>
