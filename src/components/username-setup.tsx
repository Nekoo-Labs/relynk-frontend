...
export function UsernameSetup({
  ...
  const { createProfile, isPending, isSuccess, useIsUsernameAvailable } =
    useProfileRegistry();
  ...
  const { data: isAvailable, isLoading: isCheckingUsername } =
    useIsUsernameAvailable(usernameCheck);
    ...
    <div className={isModal ? "" : "flex items-center justify-center p-4"}>
      <Card
        className={`w-full max-w-lg mx-auto ${
          isModal
            ? "glow-soft shimmer border-2 border-main/20 bg-gradient-to-br from-main/5 to-main/10"
            : "border border-border bg-main/5"
        }`}
      >
        <CardContent className="p-8 space-y-6">
          ...
          <div className="text-center space-y-3">
            <div
              className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center ${
                isModal
                  ? "bg-gradient-to-br from-main/20 to-main/10"
                  : "bg-main/20"
              }`}
            >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {["creator-hub", "my-links", "pay-me", "link-tree"].map(
                  (suggestion) => (
                    <button
                      key={suggestion}
          ...
          <div className="space-y-3">
            <Button
              ...
              className={`w-full text-main-foreground h-12 ${
                isModal
                  ? "bg-gradient-to-r from-main to-main/80 hover:from-main/90 hover:to-main/70 shadow-shadow glow-hover hover:scale-105 transition-all duration-300"
            ...
            {showSkipButton && onSkip && (
              <Button
                onClick={onSkip}
                variant="outline"
                className={`w-full border-border hover:bg-secondary-background ${
                  isModal ? "transition-all duration-300" : ""
                }`}
